...
const mockInvoices = [
  { id: '1', client: 'TechCorp', amount: 1200, status: 'Paid', date: '2024-07-15' },
  { id: '2', client: 'Innovate LLC', amount: 3500, status: 'Due', date: '2024-08-01' },
  { id: '3', client: 'DesignCo', amount: 850, status: 'Overdue', date: '2024-06-20' },
];
...
const InvoiceItem = ({ item }) => (
    <View style={styles.card}>
        ...
        <View style={{alignItems: 'flex-end'}}>
            ...
            <View style={[styles.statusPill, {backgroundColor: statusStyles[item.status].bg}]}>
                <Text style={[styles.statusText, {color: statusStyles[item.status].text}]}>{item.status}</Text>
            </View>
        </View>
    </View>
);
...
const styles = StyleSheet.create({
    ...
    addButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#007AFF', paddingVertical: 8, paddingHorizontal: 14, borderRadius: 20 },
    addButtonText: { color: '#FFFFFF', fontWeight: '600', marginLeft: 6 },
    card: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFFFFF', padding: 20, borderRadius: 12, marginBottom: 12 },
